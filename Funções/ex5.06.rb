...
def valorPagamento(valorPrest, diasAtr)
	...
	if diasAtr > 0
		...
		return (multa + valorPrest + juros).to_f
	else
		return (valorPrest).to_f
	end
end
...
while i == 0
	...
	valorPrest = gets.chomp.to_i
	print "Digite o numero de dias de atraso: "
	diasAtr = gets.chomp.to_i
	puts "Total a pagar: R$ #{(valorPagamento(valorPrest, diasAtr)).round(2)}"
